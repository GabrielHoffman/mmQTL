...
df_sort_sub = getGeneSymbol(df_sort_sub , "Gene")
...
res.lst = mclapply( 1:nrow(df_sort_sub[prod > 0,]), function(i){
...
lim = -log10(with(info, min(c(q.cit.forward, q.cit.reverse))))

ggplot(info, aes(-log10(q.cit.forward), -log10(q.cit.reverse))) + geom_point() + theme_classic() + geom_abline(color="red") + theme(aspect.ratio=1) + xlim(0, lim) + ylim(0, lim)
