...
df_eqtl = fread(synGet('syn25592272')$path, select = c('Gene', 'Variant', 'eQTL_order', 'p_fixed', 'p_random'))
setkey(df_eqtl, "Variant")

# merge with position
# only keep SNPs
df_eqtl = merge(df_eqtl, df_positions[nchar(REF) == nchar(ALT),1:3], by="Variant")

# get MAF for each variant
df_maf = fread(synGet('syn25953082')$path)

# merge with MAF
df_eqtl = merge(df_eqtl, df_maf[MAF > 0.2,], by.x="Variant", by.y="SNP")

# Select variants with large p-values
df_eqtl_subset = df_eqtl[,.SD[which.min(p_fixed),],by="Variant"]
...
df_cat2 = lapply( idx_gene[1:10], function(i){
  ...
  df = df_eqtl[(Gene == ensGene),][(eQTL_order==1),][order(position_hg19),]
