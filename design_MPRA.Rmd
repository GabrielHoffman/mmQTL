...
# merge with MAF
df_eqtl = merge(df_eqtl_all, df_maf[MAF > 0.2,], by.x="Variant", by.y="SNP")
...
# just primary eQTL
df_eqtl_1 = df_eqtl[(eQTL_order==1),]
...
df_one_annot = merge(df_one, df_eqtl_all[eQTL_order==1,c("Variant", "Gene", "HighAllele")], by=c("Variant", "Gene"))
...
sapply( names(idx.lst), function(n){
  ...
  write.table(df_one_annot[idx,], file = file, sep = '\t', row.names=FALSE, quote=FALSE)
  file.gz = R.utils::gzip(file, overwrite=TRUE)
